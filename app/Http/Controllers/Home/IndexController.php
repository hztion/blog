...
class IndexController extends Controller
{
    ...
    public function article($id, Request $request, Article $articleModel, Comment $commentModel)
    {
        ...
        $current = '';
        $topTitle = $data['title'];
        $assign = compact('category_id', 'data', 'prev', 'next', 'comment', 'current', 'topTitle');
     ...
     * @return mixed
     */
    public function archives(Article $articleModel)
    {
        // 获取文章列表数据
        $data = $articleModel->getArchivesList();
        $assign = [
            'data' => $data,
            'current' => 'archives',
            'topTitle' => 'archives',
        ];
        return view('home.index.archives', $assign);
    }
    ...
    public function catlist()
    {
        ...
        return view('home.index.catlist', $assign);
    }
     ...
     * @return mixed
     */
    public function tags(Article $articleModel)
    {
        $tags = Tag::withTrashed()->get();
        $data = $articleModel->getTagsList($tags);
        $assign = [
            'data' => $data,
            'tags' => $tags,
            'current' => 'tags',
